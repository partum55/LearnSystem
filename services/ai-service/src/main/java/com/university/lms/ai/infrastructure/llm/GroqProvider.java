...
/**
 * LLM Provider implementation for Groq API (OpenAI-compatible). Primary provider with highest
 * priority.
 */
@Component
...
public class GroqProvider implements LLMProvider {
  ...
  public LLMResponse generate(String prompt, String systemPrompt, LLMGenerationOptions options, String apiKey) {
    ...
    try {
      ...
      String model =
          options.getModel() != null ? options.getModel() : llamaApiProperties.getModel();
      ...
      throw new LLMProviderException(PROVIDER_NAME, "Groq API error: " + e.getStatusCode(), e);
    } catch (Exception e) {
      log.error("Groq generation failed", e);
      throw new LLMProviderException(
          PROVIDER_NAME, "Failed to generate with Groq: " + e.getMessage(), e);
    }
  }
  ...
  private LLMResponse parseResponse(String response, long startTime, String model) {
    try {
      ...
      throw new LLMProviderException(PROVIDER_NAME, "Failed to parse Groq response", e);
    }
  }
}
