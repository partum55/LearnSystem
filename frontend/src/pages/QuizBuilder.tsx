...
export const QuizBuilder: React.FC = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const courseList = Array.isArray(data) ? data : (data as { content?: Course[] }).content || [];
      const normalized = (Array.isArray(courseList) ? courseList : []).map((course: Course & {
        titleUk?: string;
        titleEn?: string;
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 mb-6">
            <nav className="flex gap-8">
              <button
                ...
                className={`py-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'basic'
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setQuiz({
                          ...quiz,
                          settings: { ...quiz.settings, show_results_after: e.target.value as 'immediate' | 'after_due' | 'manual' }
                        })}
                        className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-2 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500"
                      >
