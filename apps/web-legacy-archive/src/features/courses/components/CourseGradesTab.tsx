...
export const CourseGradesTab: React.FC<CourseGradesTabProps> = ({ courseId }) => {
  ...
  const fetchGradebook = async () => {
    ...
    setError(null);
    try {
      const response = await canonicalGradebookApi.getStudent(courseId);
      const modules: ModuleGrades[] = response.modules.map((module, index) => ({
        module_id: module.moduleId,
        module_title: module.title,
        module_position: index,
        grades: module.assignments.map((assignment) => ({
          assignment_id: assignment.assignmentId,
          assignment_title: assignment.title,
          assignment_type: assignment.type,
          max_points: Number(assignment.maxPoints || 0),
          score: assignment.points === null || assignment.points === undefined ? undefined : Number(assignment.points),
          percentage:
          ...
          is_late: false,
          is_excused: false,
          notes: assignment.comment || undefined,
        })),
      }));
      ...
      const mapped: GradebookData = {
        ...
        summary: {
          current_grade: response.total.percentage ?? undefined,
          letter_grade:
            response.total.percentage === undefined || response.total.percentage === null ? '-' :
          ...
          assignments_completed: modules.flatMap((module) => module.grades).filter((grade) => grade.score !== undefined).length,
          assignments_total: modules.flatMap((module) => module.grades).length,
        },
        modules,
      };
