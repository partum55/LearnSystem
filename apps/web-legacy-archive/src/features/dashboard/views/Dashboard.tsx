...
import { ProgressItem } from '@/features/grades/api/progress';
...
export const Dashboard: React.FC = () => {
  ...
  // TODO canonical API: dedicated progress item feed is not exposed yet.
  useEffect(() => {
    setProgressItems([]);
  ...
  useEffect(() => {
    ...
    setTeacherTodo(null);
    setStudentReminders([]);
  }, [isInstructorRole, isStudentRole, studentDashboard]);
  ...
  const dashboardDeadlines = studentDashboard?.upcomingDeadlines || [];
  const upcomingDeadlines = (dashboardDeadlines.length > 0 ? dashboardDeadlines.map((item) => ({
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {isInstructorRole && teacherTodo && (
          ...
          >
            <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <Button size="sm" variant="secondary" onClick={() => router.push('/teacher/todo')}>
        ...
        {widgets.filter(w => w.visible).length > 0 ? (
          ...
          >
            ...
            <Button onClick={() => router.push('/dashboard/customize')} size="sm">
