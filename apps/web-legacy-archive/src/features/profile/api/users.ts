...
interface ApiUser {
  ...
  lastName?: string;
  studentId?: string;
  role: 'SUPERADMIN' | 'ADMIN' | 'TEACHER' | 'USER' | 'STUDENT' | 'TA';
  locale?: 'UK' | 'EN';
  theme?: 'light' | 'dark';
  avatarUrl?: string;
  bio?: string;
...
const mapApiUserToFrontend = (u: ApiUser): User => {
  ...
  if (u.role === 'SUPERADMIN' || u.role === 'ADMIN' || u.role === 'TEACHER' || u.role === 'USER') {
    mappedRole = u.role as UserRole;
  } else if (u.role === 'STUDENT' || u.role === 'TA') {
    mappedRole = 'USER';
  }
  return {
    id: u.id,
