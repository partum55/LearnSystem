...
export const AssignmentSubmissionPanel: React.FC<AssignmentSubmissionPanelProps> = ({
  ...
  const loadOrCreateSubmission = async () => {
    ...
    try {
      const assignment = await canonicalAssignmentsApi.get(assignmentId);
      if (assignment.studentState?.submissionId) {
        hydrateFromSubmission(parseSubmission({
  ...
  const saveDraft = async () => {
    ...
    setSavingDraft(true);
    try {
      await canonicalSubmissionsApi.edit(submission.id, {
        text: normalizedType === 'CODE' ? undefined : textAnswer,
        url: normalizedType === 'URL' ? urlAnswer : undefined,
        code: normalizedType === 'CODE' ? codeAnswer : undefined,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const request = {
        text: normalizedType === 'TEXT' ? textAnswer : undefined,
        code: normalizedType === 'CODE' ? codeAnswer : undefined,
        url: normalizedType === 'URL' ? urlAnswer : undefined,
        programmingLanguage: normalizedType === 'CODE' ? programmingLanguage : undefined,
        files: normalizedType === 'FILE_UPLOAD'
      ...
      const response =
        normalizedType === 'CODE'
          ? await canonicalSubmissionsApi.submitVpl(assignmentId, request)
          : normalizedType === 'FILE_UPLOAD'
            ? await canonicalSubmissionsApi.submitFile(assignmentId, request)
            : await canonicalSubmissionsApi.submitRte(assignmentId, request);
      hydrateFromSubmission(parseSubmission(response as unknown as Record<string, unknown>));
