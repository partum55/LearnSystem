...
import com.university.lms.submission.client.AssessmentClient;
...
public class SubmissionService {
    ...
    private void applyLateFlags(Submission submission, LocalDateTime submittedAt) {
        try {
            AssessmentClient.AssignmentDto assignment = assessmentClient.getAssignment(submission.getAssignmentId());
            if (assignment == null || assignment.dueDate() == null || submittedAt == null) {
                submission.setIsLate(false);
                submission.setDaysLate(0);
                return;
            }

            if (submittedAt.isAfter(assignment.dueDate())) {
                long minutesLate = Duration.between(assignment.dueDate(), submittedAt).toMinutes();
