...
/**
 * Semantic caching service for AI responses using Redis. Caches responses based on prompt hash to
...
public class AISemanticCacheService {
   ...
   */
  public Optional<LLMResponse> get(
      String prompt, String systemPrompt, LLMGenerationOptions options) {
    if (!cacheEnabled) {
   ...
   */
  public void put(
      String prompt, String systemPrompt, LLMGenerationOptions options, LLMResponse response) {
    if (!cacheEnabled) {
      return;
    }
    ...
    try {
      ...
      log.debug(
          "Cached response with key: {}, TTL: {} minutes",
          cacheKey.substring(CACHE_PREFIX.length()),
          cacheTtlMinutes);
