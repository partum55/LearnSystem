...
public class AICostTrackingService {
  ...
  public void recordUsage(String userId, int promptTokens, int completionTokens) {
    ...
    try {
      ...
      log.debug(
          "Recorded usage for user {}: +{} tokens, total: {}",
          userId,
  ...
  public List<AIUserUsage> getTopUsers(int limit) {
    return usageRepository.findTopUsersByPeriod(
        AIUserUsage.currentPeriod(), PageRequest.of(0, limit));
  }
    ...
    return getCurrentUsage(userId)
        .map(
            usage -> {
              double percentUsed =
                  monthlyTokenLimit > 0
                      ? (double) usage.getTotalTokens() / monthlyTokenLimit * 100
                      : 0.0;
              long remainingTokens = Math.max(0, monthlyTokenLimit - usage.getTotalTokens());
        ...
        .orElseGet(
            () -> new UserUsageInfo(userId, 0, 0, 0, BigDecimal.ZERO, 0.0, monthlyTokenLimit));
  }

  public record UsageSummary(
      String period, long totalTokens, BigDecimal totalCost, long totalRequests) {}
  ...
  public AIUsageSummaryResponse getSystemSummary() {
    ...
    long failedRequests = users.stream().mapToInt(AIUserUsage::getFailedRequestCount).sum();
  ...
  public AIUsageStatsResponse getUsageStats() {
    ...
    for (Object[] row : providerTokens) {
      ...
      byProvider.put(
          provider,
          AIUsageStatsResponse.ProviderStats.builder().provider(provider).tokens(tokens).build());
    }
    ...
    for (Object[] row : contentTypeStats) {
      ...
      byContentType.put(
          contentType,
          AIUsageStatsResponse.ContentTypeStats.builder()
              .contentType(contentType)
              .requests(requests)
              .build());
    }

    // Get latency percentiles (simplified - actual implementation would use proper percentile
    // calculation)
    Double avgLatency = logRepository.getAverageLatency();
