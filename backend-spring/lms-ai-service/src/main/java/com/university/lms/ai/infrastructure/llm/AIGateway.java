...
/**
 * Central gateway for all LLM operations. Provides multi-provider routing with automatic failover
...
public class AIGateway {
  ...
  public AIGateway(
    ...
    log.info(
        "AIGateway initialized with {} providers: {}",
        providers.size(),
    ...
    for (LLMProvider provider : orderedProviders) {
      ...
      try {
        ...
        LLMResponse response =
            circuitBreaker.executeSupplier(() -> provider.generate(prompt, systemPrompt, options));
        ...
        log.info(
            "Generation successful with provider: {}, latency: {}ms",
            providerName,
  ...
  public record ProviderStatus(
      String name, int priority, boolean available, String circuitBreakerState, String health) {}
}
