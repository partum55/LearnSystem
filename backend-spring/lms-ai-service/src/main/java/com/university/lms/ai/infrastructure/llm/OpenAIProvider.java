 ...
 * DISABLED - Only Groq is used as the LLM provider.
 */
        ...
        try {
            ...
            if (e.getStatusCode().value() == 429) {
                throw new LLMProviderException(PROVIDER_NAME,
                        "OpenAI rate limit exceeded", e, true);
            }

            throw new LLMProviderException(PROVIDER_NAME,
                    "OpenAI API error: " + e.getStatusCode(), e);
        } catch (Exception e) {
            log.error("OpenAI generation failed", e);
            throw new LLMProviderException(PROVIDER_NAME,
                    "Failed to generate with OpenAI: " + e.getMessage(), e);
        }
    }
