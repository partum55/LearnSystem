...
public class PredictionService {
  ...
  public PredictionResponseDto getStudentPredictions(PredictionRequestDto request) {
    ...
    List<StudentPredictionDto> predictions =
        request.getStudents().stream()
            .map(
                student -> {
                  ...
                  double averageGrade =
                      student.getGrades().stream().mapToDouble(d -> d).average().orElse(0);
                  ...
                  return new StudentPredictionDto(
                      student.getStudentId(), predictedGrade, confidence);
                })
