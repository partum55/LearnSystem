...
public interface AIGenerationLogRepository extends JpaRepository<AIGenerationLog, String> {
  ...
  @Query(
      "SELECT l.contentType, COUNT(l) FROM AIGenerationLog l WHERE l.success = true GROUP BY l.contentType")
  List<Object[]> countSuccessfulByContentType();

  /** Get total token usage by provider */
  @Query(
      "SELECT l.provider, SUM(l.promptTokens), SUM(l.completionTokens) FROM AIGenerationLog l GROUP BY l.provider")
  List<Object[]> getTokenUsageByProvider();

  /** Get average latency by provider */
  @Query(
      "SELECT l.provider, AVG(l.latencyMs) FROM AIGenerationLog l WHERE l.success = true GROUP BY l.provider")
  ...
  @Query(
      "SELECT l.provider, COUNT(l) FROM AIGenerationLog l WHERE l.success = false GROUP BY l.provider")
