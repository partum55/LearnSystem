...
public class CoursePersistenceService {
    ...
    public Map<String, Object> saveGeneratedCourse(GeneratedCourseResponse response, UUID ownerId, String authToken) {
        ...
        try {
            // 1. Create the course
            AiCourseCreateRequest courseRequest = persistenceMapper.toCourseCreateRequest(validated.getCourse(), ownerId);
                ...
                for (GeneratedCourseResponse.ModuleData moduleData : validated.getModules()) {
                    ...
                    if (moduleData.getAssignments() != null) {
                        for (GeneratedCourseResponse.AssignmentData assignmentData : moduleData.getAssignments()) {
                            Map<String, Object> savedAssignment = createAssignment(courseId, moduleId, assignmentData, authToken);
                            UUID assignmentId = UUID.fromString(savedAssignment.get("id").toString());
                            context.assignments.add(new PersistedAssignment(assignmentId, moduleId, assignmentData, savedAssignment));
                        }
                    }
            ...
            return Map.of(
                "course", context.savedCourse,
                "modules", context.modules.stream().map(PersistedModule::savedModule).toList(),
                "verification", verification.toMap(),
                "message", verification.matches ? "Course successfully created with all content" :
    ...
    private Map<String, Object> createModule(UUID courseId, GeneratedCourseResponse.ModuleData moduleData, String authToken) {
        AiModuleCreateRequest moduleRequest = persistenceMapper.toModuleCreateRequest(courseId, moduleData);
        WebClient webClient = webClientBuilder.baseUrl(courseServiceUrl + "/api").build();
    ...
    private Map<String, Object> createAssignment(UUID courseId, UUID moduleId,
        ...
        Map<String, Object> savedAssignment = webClient.post()
            .uri("/assignments")
            .header("Authorization", authToken)
            .contentType(MediaType.APPLICATION_JSON)
    ...
    private PersistedQuiz createQuiz(UUID courseId, GeneratedCourseResponse.QuizData quizData,
                                     UUID createdBy, String authToken) {
        WebClient webClient = webClientBuilder.baseUrl(assessmentServiceUrl + "/api").build();

        Map<String, Object> savedQuiz = webClient.post()
            .uri(uriBuilder -> uriBuilder.path("/quizzes")
    ...
    private void updateQuizSettings(UUID quizId, GeneratedCourseResponse.QuizData quizData, String authToken) {
    ...
    private PersistedQuestion createQuizQuestion(UUID courseId, UUID quizId,
                                                 GeneratedCourseResponse.QuestionData questionData,
                                                 int position, String authToken) {
        AiQuestionCreateRequest questionRequest = persistenceMapper.toQuestionCreateRequest(courseId, questionData);
        WebClient webClient = webClientBuilder.baseUrl(assessmentServiceUrl + "/api").build();

        Map<String, Object> savedQuestion = webClient.post()
            .uri("/questions")
            .header("Authorization", authToken)
            .contentType(MediaType.APPLICATION_JSON)
    ...
    private VerificationResult verifyPersistedCourse(PersistenceContext context,
                                                     GeneratedCourseResponse source,
                                                     String authToken) {
        ...
        compareField(result, "course.titleUk", source.getCourse().getTitleUk(), persistedCourse.get("titleUk"));
        compareField(result, "course.titleEn", source.getCourse().getTitleEn(), persistedCourse.get("titleEn"));
        compareField(result, "course.descriptionUk", source.getCourse().getDescriptionUk(), persistedCourse.get("descriptionUk"));
        compareField(result, "course.descriptionEn", source.getCourse().getDescriptionEn(), persistedCourse.get("descriptionEn"));

        for (PersistedModule module : context.modules) {
            Map<String, Object> persistedModule = fetchModule(context.courseId, module.moduleId, authToken);
            compareField(result, "modules[" + module.moduleId + "].title", module.source.getTitle(), persistedModule.get("title"));
            compareField(result, "modules[" + module.moduleId + "].description", module.source.getDescription(), persistedModule.get("description"));
        }
        ...
        for (PersistedAssignment assignment : context.assignments) {
            ...
            compareField(result, "assignments[" + assignment.assignmentId + "].title", assignment.source.getTitle(), persistedAssignment.get("title"));
            compareField(result, "assignments[" + assignment.assignmentId + "].description", assignment.source.getDescription(), persistedAssignment.get("description"));
            compareField(result, "assignments[" + assignment.assignmentId + "].instructions", assignment.source.getInstructions(), persistedAssignment.get("instructions"));
            compareField(result, "assignments[" + assignment.assignmentId + "].assignmentType", assignment.source.getAssignmentType(), persistedAssignment.get("assignmentType"));
        }

        for (PersistedQuiz quiz : context.quizzes) {
            Map<String, Object> persistedQuiz = fetchQuiz(quiz.quizId, authToken);
            compareField(result, "quizzes[" + quiz.quizId + "].title", quiz.source.getTitle(), persistedQuiz.get("title"));
            compareField(result, "quizzes[" + quiz.quizId + "].description", quiz.source.getDescription(), persistedQuiz.get("description"));
            compareField(result, "quizzes[" + quiz.quizId + "].timeLimit", quiz.source.getTimeLimit(), persistedQuiz.get("timeLimit"));
            compareField(result, "quizzes[" + quiz.quizId + "].attemptsAllowed", quiz.source.getAttemptsAllowed(), persistedQuiz.get("attemptsAllowed"));
            compareField(result, "quizzes[" + quiz.quizId + "].shuffleQuestions", quiz.source.getShuffleQuestions(), persistedQuiz.get("shuffleQuestions"));
            ...
            for (PersistedQuestion question : quiz.questions) {
                ...
                compareField(result, "questions[" + question.questionId + "].stem", question.source.getQuestionText(), persistedQuestion.get("stem"));
                compareField(result, "questions[" + question.questionId + "].questionType", question.source.getQuestionType(), persistedQuestion.get("questionType"));
            }
        }
        ...
        if (result.mismatches.isEmpty()) {
            ...
            log.warn("AI persistence verification found {} mismatches for course {}", result.mismatches.size(), context.courseId);
        }
        return result;
    }
    ...
    private void rollbackCreatedResources(PersistenceContext context, String authToken) {
        WebClient courseClient = webClientBuilder.baseUrl(courseServiceUrl + "/api").build();
        WebClient assessmentClient = webClientBuilder.baseUrl(assessmentServiceUrl + "/api").build();
        ...
        for (PersistedModule module : context.modules) {
            deleteSafely(courseClient, "/courses/{courseId}/modules/{moduleId}", context.courseId, module.moduleId, authToken);
        }
    ...
    private void deleteSafely(WebClient client, String uriTemplate, UUID resourceId, String authToken) {
        deleteSafely(client, uriTemplate, resourceId, null, authToken);
    }

    private void deleteSafely(WebClient client, String uriTemplate, UUID courseId, UUID resourceId, String authToken) {
        try {
            client.delete()
                .uri(uriBuilder -> {
                    if (resourceId == null) {
    ...
    private void compareField(VerificationResult result, String field, Object expected, Object actual) {
    ...
    private record PersistedModule(UUID moduleId, GeneratedCourseResponse.ModuleData source,
                                   Map<String, Object> savedModule) {
    }

    private record PersistedAssignment(UUID assignmentId, UUID moduleId,
    ...
    private static class PersistedQuiz {
        ...
        private PersistedQuiz(UUID quizId, GeneratedCourseResponse.QuizData source, Map<String, Object> savedQuiz) {
            this.quizId = quizId;
            this.source = source;
    ...
    private record PersistedQuestion(UUID questionId, GeneratedCourseResponse.QuestionData source,
