...
public interface PromptABTestRepository extends JpaRepository<PromptABTest, String> {
     ...
     */
    @Query("SELECT p.variantName, COUNT(p), SUM(CASE WHEN p.success = true THEN 1 ELSE 0 END) " +
     ...
     */
    @Query("SELECT p.variantName, AVG(p.latencyMs) FROM PromptABTest p " +
     ...
     */
    @Query("SELECT p.variantName, AVG(p.qualityScore) FROM PromptABTest p " +
     ...
     */
    @Query("SELECT p.variantName, AVG(p.userRating) FROM PromptABTest p " +
     ...
     */
    @Query("SELECT p.variantName, SUM(p.totalTokens) FROM PromptABTest p " +
