...
/**
 * Resilience4j configuration for AI service resilience patterns.
 * Configures Circuit Breaker, Retry, and Time Limiter for LLM API calls.
 */
@Configuration
@Slf4j
public class Resilience4jConfig {
    ...
    public CircuitBreakerRegistry circuitBreakerRegistry() {
        CircuitBreakerConfig llamaApiConfig = CircuitBreakerConfig.custom()
        ...
        registry.circuitBreaker(LLAMA_API).getEventPublisher()
                .onStateTransition(event ->
                    log.warn("Circuit Breaker '{}' state transition: {} -> {}",
                            LLAMA_API, event.getStateTransition().getFromState(),
     ...
     * Configuration:
     * - Max attempts: 3
     * - Wait duration: 2 seconds (exponential backoff)
     */
    @Bean
    public RetryRegistry retryRegistry() {
        RetryConfig llamaApiConfig = RetryConfig.custom()
                .maxAttempts(3)
        ...
        registry.retry(LLAMA_API).getEventPublisher()
                .onRetry(event ->
                    log.warn("Retry attempt {} for '{}': {}",
                            event.getNumberOfRetryAttempts(), LLAMA_API, event.getLastThrowable().getMessage()));
    ...
    public TimeLimiterRegistry timeLimiterRegistry() {
        TimeLimiterConfig llamaApiConfig = TimeLimiterConfig.custom()
